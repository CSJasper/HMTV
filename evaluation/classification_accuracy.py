...
def main(args):
    ...
    num_total_video=len(val_data.video_list)
    ...
    prec1 = validate(
        val_loader, device, 
        model_full, config, classes_features, args.test_crops, args.test_clips,num_total_video)
...
def validate(val_loader, device, model, config, text_features, test_crops, test_clips,num_total_video):
    ...
    proc_start_time = time.time()
    result_acc=0
    total=num_total_video
    with torch.no_grad():
        n_class = text_features.size(0)
        
        for i, (image, class_id,name) in enumerate(val_loader):
            ...
            image_features = model.module.encode_image(image_input)
            logits = model(image_input,text_features)
            result=torch.max(logits,1)
            for res in result.indices:
                if res.cpu().detach().numpy() in config.data.dif_label:
                    result_acc+=1
        print(result_acc/total*100, " is acc")
        print("acc num : ", result_acc)
        return 0
